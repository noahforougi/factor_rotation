...
def cycle_classifier(df: pd.DataFrame):
    ...
    for i in range(len(peaks)):
        if i == 0 and troughs[0] < peaks[0]:
            X.iloc[
                troughs[0] : peaks[0], X.columns.get_loc("Cycle_Phase")
            ] = "Expansion"
        elif i < len(troughs):
            X.iloc[
                troughs[i] : peaks[i], X.columns.get_loc("Cycle_Phase")
            ] = "Expansion"
            X.iloc[
                peaks[i] : troughs[i + 1] if i + 1 < len(troughs) else len(X),
                X.columns.get_loc("Cycle_Phase"),
            ] = "Contraction"
...
def calculate_portfolio_returns(asset_returns, cycle_weights, equal_weights):
    ...
    equal_weighted_returns = asset_returns[list(equal_weights.keys())] * pd.Series(
        equal_weights
    )
    equal_weighted_returns[
        "Equal_Weight_Portfolio_Returns"
    ] = equal_weighted_returns.sum(axis=1)
    ...
    comparison = pd.DataFrame(
        {
            ...
            "equal_weight_returns": equal_weighted_returns[
                "Equal_Weight_Portfolio_Returns"
            ],
        }
    )
...
def create_strategy(factor_prices: pd.DataFrame, bci: pd.DataFrame) -> pd.DataFrame:
    ...
    X = bci.assign(date=lambda x: pd.to_datetime(x.date)).merge(
        monthly_factor_returns, on="date", how="inner"
    )

    # Calculate and compare the returns
    comparison = calculate_portfolio_returns(
        X, config.WEIGHTS, config.EQUAL_WEIGHTS
    ).assign(date=lambda x: pd.to_datetime(x.date))
    comparison = comparison.merge(
        bci.assign(date=lambda x: pd.to_datetime(x.date)), on=["date"], how="inner"
    )
